...
def application(environ, start_response):
    ...
    if path == '/register' and un and pw:
        ...
        if user:
            ...
            headers.append(('Set-Cookie','{}-score={}:{}'.format(un, correct, wrong)))
            start_response('200 OK', headers)
        ...
        if user:
            ...
            cookie_name = un + "-score"
            correct = int(cookies[cookie_name].value.split(':')[0])
            wrong = int(cookies[cookie_name].value.split(':')[1])
            if 'factor1' in params and 'factor2' in params and 'answer' in params:
                if int(params['answer'][0]) == int(params['factor1'][0])* int(params['factor2'][0]):
                    correct += 1
                    headers.append(('Set-Cookie', '{}={}:{}; expires= Sat, 04 July 2020 01:00:00 GMT'.format(cookie_name, correct, wrong)))
                    return["<p style = 'background-color: lightgreen'><a href='/account'>Correct.</a></p>".encode()]
                else:
                    wrong += 1
                    headers.append(('Set-Cookie', '{}={}:{}; expires= Sat, 04 July 2020 01:00:00 GMT'.format(cookie_name, correct, wrong)))
                    return ["<p style = 'background-color: red'><a href='/account'>Incorrect.</a></p>".encode()]
