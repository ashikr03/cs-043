...
def application(environ, start_response):
    ...
    if path == '/register' and un and pw:
        ...
        if user:
            ...
            return ["Sorry, username {} is taken. <a href='/register'>Register</a>".format(un).encode()]
        else:
        ...
        user = cursor.execute('SELECT* FROM users WHERE username = ? AND password = ?', [un, pw])
        if user:
            headers.append(('Set-Cookie', 'session = {}:{}'.format(un,pw)))
            start_response('200 OK', headers)
        ...
        headers.append(('Set-Cookie', 'session=0; expires= Thu, 01 Jan 1970 00:00 GMT'))
        start_response('200 OK', headers)
        ...
        if user:
            ...
            return['Not logged in {}. <a href="/">Login</a>'.format(un).encode()]
    elif path == '/':
        start_response('200 OK', headers)
        page = '''<!DOCTYPE html>
